...
module Idp
  ...
  class MockClient
    DOCUMENT_ID_PATTERN = /\A[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}\z/i
    ...
    def document_path(id)
      storage_dir.join("#{validated_document_id(id)}.json")
    end

    def validated_document_id(id)
      value = id.to_s
      return value if DOCUMENT_ID_PATTERN.match?(value)

      raise Idp::Error, "Invalid document id #{id.inspect}"
    end
