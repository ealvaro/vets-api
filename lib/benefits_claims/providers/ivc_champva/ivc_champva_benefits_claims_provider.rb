  ...
  module Providers
    module IvcChampva
      class IvcChampvaBenefitsClaimsProvider
        ...
        def get_claim(id)
          raise Common::Exceptions::RecordNotFound, id if user_emails.blank?

          records = scoped_forms.where(form_uuid: id).order(:created_at)
          raise Common::Exceptions::RecordNotFound, id if records.blank?
        ...
        def transform_to_dto(records)
          dto = ClaimBuilder.build_claim_response(records, @user)
        ...
        def user_emails
          @user_emails ||= begin
            verifications = @user&.user_account&.user_verifications
            verification_emails = verifications&.includes(:user_credential_email)&.filter_map do |verification|
