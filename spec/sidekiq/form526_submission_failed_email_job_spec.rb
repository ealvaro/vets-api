...
RSpec.describe Form526SubmissionFailedEmailJob, type: :job do
  ...
  describe '#perform' do
    ...
    it 'sends a submission failed email using submission data' do
    ...
    context 'when source is pif' do
      ...
      it 'capitalizes the first name with a fallback greeting' do
        expect(email_service).to receive(:send_email).with(expected_params)

        subject.new.perform(form526_submission.id, 'pif')
      end
    end
  end
end
