...
RSpec.describe Form214192SubmissionEmailJob, type: :job do
  subject(:perform_job) { described_class.new.perform(claim.id) }

  let(:claim) { create(:va214192) }
  ...
  describe '#perform' do
    context 'when successful' do
      it 'looks up MPI profile and sends VA Notify email to Veteran ICN' do
        ...
        expect(notify_client).to have_received(:send_email).with(
          ...
          template_id: Settings.vanotify.services.bio_aquia.template_id.form_4192_submission_email,
          personalisation: {
            'first_name' => 'John',
        ...
        expect(StatsD).to have_received(:increment).with('api.form214192.email.sent')
        expect(Rails.logger).to have_received(:info).with(
          'Form214192SubmissionEmailJob confirmation email sent',
    ...
    context 'when claim not found' do
      it 'logs warning, tracks metric, and does not retry' do
        ...
        expect(StatsD).to have_received(:increment).with('api.form214192.email.claim_not_found')
        expect(Rails.logger).to have_received(:warn).with(
          'Form214192SubmissionEmailJob claim not found',
    ...
    context 'when template ID is missing' do
      before do
        allow(Settings.vanotify.services.bio_aquia.template_id).to receive(:form_4192_submission_email)
          .and_return(nil)
      end
      ...
      it 'does not send email and tracks missing template metric' do
        ...
        expect(notify_client).not_to have_received(:send_email)
        expect(StatsD).to have_received(:increment).with('api.form214192.email.missing_template_id')
        expect(Rails.logger).to have_received(:error).with(
          'Form214192SubmissionEmailJob missing template_id for form_4192_submission_email'
        )
      end
    end
    ...
    context 'when API key is missing' do
      ...
      it 'does not send email and tracks missing_api_key metric' do
        ...
        expect(notify_client).not_to have_received(:send_email)
        expect(StatsD).to have_received(:increment).with('api.form214192.email.missing_api_key')
        expect(Rails.logger).to have_received(:error).with(
          'Form214192SubmissionEmailJob missing vanotify api_key'
        )
      end
    end
    ...
    context 'when MPI lookup fails' do
      ...
      it 'does not send email and tracks mpi_lookup_failure metric' do
        ...
        expect(notify_client).not_to have_received(:send_email)
        expect(StatsD).to have_received(:increment).with('api.form214192.email.mpi_lookup_failure')
        expect(Rails.logger).to have_received(:error).with(
          'Form214192SubmissionEmailJob MPI lookup failed',
          hash_including(saved_claim_id: claim.id, claim_guid: claim.guid)
        )
      end
    end
    ...
    context 'when MPI lookup raises an exception' do
      ...
      it 'does not send email and tracks mpi_lookup_error metric' do
        ...
        expect(notify_client).not_to have_received(:send_email)
        expect(StatsD).to have_received(:increment).with('api.form214192.email.mpi_lookup_error')
        expect(Rails.logger).to have_received(:error).with(
          'Form214192SubmissionEmailJob MPI lookup error',
          {
            error: 'StandardError',
            saved_claim_id: claim.id
          }
        )
        expect(StatsD).not_to have_received(:increment).with('api.form214192.email.mpi_lookup_failure')
      end
    end
    ...
    context 'when VA Notify send_email fails' do
      ...
      it 'tracks failure and re-raises for Sidekiq retry' do
        expect do
          perform_job
        end.to raise_error(Common::Exceptions::BackendServiceException)

        expect(StatsD).to have_received(:increment).with('api.form214192.email.send_failure')
        expect(Rails.logger).to have_received(:error).with(
          'Form214192SubmissionEmailJob failed to send confirmation email',
          hash_including(
