...
RSpec.describe SavedClaim::EducationBenefits::VA1990 do
  ...
  describe '#after_submit' do
    ...
    describe 'confirmation email for the 1990' do
      ...
      context 'when va_notify_v2_form1990_confirmation_email is disabled' do
        before do
          allow(Flipper).to receive(:enabled?).with(:va_notify_v2_form1990_confirmation_email).and_return(false)
        end
      ...
      context 'when va_notify_v2_form1990_confirmation_email is enabled' do
        before do
          allow(Flipper).to receive(:enabled?).with(:va_notify_v2_form1990_confirmation_email).and_return(true)
          allow(VANotify::V2::QueueEmailJob).to receive(:enqueue)
        end

        it 'sends email via V2 QueueEmailJob for chapter 33' do
          allow(VANotify::EmailJob).to receive(:perform_async)
        ...
        it 'sends email via V2 QueueEmailJob with relinquished' do
          allow(VANotify::EmailJob).to receive(:perform_async)
