...
RSpec.describe CoeClaimFormValidation::DateRange do
  ...
  describe '#validate_coe_date_range' do
    ...
    it 'rejects an invalid from date using the chronology format message' do
    ...
    context 'when chronology validation is disabled' do
      before do
        allow(CoeClaimFormValidation).to receive(:const_defined?)
          .with(:COE_DATE_RANGE_STRING_FORMAT_MESSAGE, false)
          .and_return(false)
      end

      it 'uses the generic invalid date message' do
        host.send(:validate_coe_date_range, { 'from' => 'not-a-date' }, '/base')
        expect(host.errors['/base/dateRange/from']).to include('must be a valid date')
      end

      it 'does not validate chronological order' do
        dr = {
          'from' => '2005-01-01T00:00:00.000Z',
          'to' => '2000-01-01T00:00:00.000Z'
        }
