  ...
  describe '#get_vitals' do
    ...
    context 'happy path' do
      context 'when data exists for both VistA + OH' do
        it 'returns all vitals' do
          ...
          # 7 records with multiple locations after filtering out weight dosing
          expect(Rails.logger).to receive(:info)
            .with(
              message: 'Multiple locations found for 7 Vital records:',
          ...
          # 17 vitals after filtering out OH "weight dosing" duplicate
          expect(vitals.size).to eq(17)
          ...
          # OH "weight dosing" record (VS-15249708684) is filtered out to avoid duplicate weight entries
          oh_weight_dosing = vitals.find { |vital| vital.id == 'VS-15249708684' }
          expect(oh_weight_dosing).to be_nil
      ...
      context 'when data exists for only VistA or OH' do
        ...
        it 'returns vitals for OH only' do
          ...
          allow_any_instance_of(UnifiedHealthData::Client)
            ...
            .and_return(build_faraday_response(modified_response))
          vitals = service.get_vitals[:records]
          # 7 vitals after filtering out OH "weight dosing" duplicate
          expect(vitals.size).to eq(7)
          expect(vitals.map(&:type)).to contain_exactly(
            'PULSE_OXIMETRY',
            'WEIGHT',
            'TEMPERATURE',
    ...
    context 'logging and metrics' do
      ...
      it 'calls log_vitals_metrics when flipper enabled' do
        ...
        # Logs filter action for both entered-in-error and weight_dosing records
        expect(Rails.logger).to have_received(:info).with(
          hash_including(
            service: 'medical_records',
            resource: 'vitals',
