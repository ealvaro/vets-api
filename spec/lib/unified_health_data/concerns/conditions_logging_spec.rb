...
require 'unified_health_data/concerns/conditions_logging'
require 'unified_health_data/constants'
require 'medical_records/medical_records_log'

RSpec.describe UnifiedHealthData::Concerns::ConditionsLogging do
  ...
  let(:test_class) do
    klass = Class.new do
      include UnifiedHealthData::Concerns::ConditionsLogging
  ...
  describe '#conditions_logging_enabled?' do
    ...
    it 'returns false when both toggles are disabled' do
      ...
      expect(instance.send(:conditions_logging_enabled?)).to be false
    end
  end

  describe '#log_conditions_response_count' do
    before do
      allow(Flipper).to receive(:enabled?)
        .with(:mhv_medical_records_conditions_diagnostic, user)
        .and_return(true)
    ...
    it 'logs the total, returned, and filtered counts' do
      instance.send(:log_conditions_response_count, 10, 7)
  ...
  describe '#log_conditions_index_metrics' do
    ...
    it 'logs the source breakdown' do
      instance.send(:log_conditions_index_metrics, combined_records, 3)
    ...
    it 'emits StatsD gauges for each source' do
      ...
      expect(StatsD).to have_received(:gauge).with('api.uhd.conditions.index.total', 3)
      expect(StatsD).to have_received(:gauge).with('api.uhd.conditions.index.vista', 2)
      expect(StatsD).to have_received(:gauge).with('api.uhd.conditions.index.oracle_health', 1)
    end
  end

  describe '#warn_conditions_high_filter_rate' do
    it 'warns when more than 50% of records are filtered' do
      instance.send(:warn_conditions_high_filter_rate, 10, 4)
    ...
    it 'emits a StatsD increment for the anomaly' do
      ...
      expect(StatsD).to have_received(:increment)
        .with('api.uhd.conditions.anomaly.high_filter_rate')
    end

    it 'includes source_breakdown when high filter rate triggers' do
      instance.send(:warn_conditions_high_filter_rate, 10, 4, source_breakdown: { vista_raw: 6, oracle_health_raw: 4 })
    ...
    it 'does not warn when filter rate is at or below 50%' do
      instance.send(:warn_conditions_high_filter_rate, 10, 5)
    ...
    it 'does not warn when raw_count is zero' do
      instance.send(:warn_conditions_high_filter_rate, 0, 0)

      expect(Rails.logger).not_to have_received(:warn)
    end
  end

  describe '#log_conditions_metrics' do
    let(:combined_records) do
      [
        { 'source' => 'vista', 'resource' => {} },
        { 'source' => 'oracle-health', 'resource' => {} }
    ...
    let(:parsed_conditions) { [double('Condition')] }
    ...
    it 'calls response count, index metrics, and high filter rate warning' do
      instance.send(:log_conditions_metrics, combined_records, parsed_conditions)
    ...
    it 'skips diagnostic logs when toggle is disabled but still warns on high filter rate' do
      ...
      instance.send(:log_conditions_metrics, combined_records, [])
  ...
  describe '#log_conditions_raw_source_counts' do
    let(:body) do
      {
    ...
    it 'logs raw entry counts when toggle is enabled' do
      ...
      instance.send(:log_conditions_raw_source_counts, body)
  ...
  describe '#log_conditions_error' do
    ...
    it 'increments StatsD error counter' do
      ...
      expect(StatsD).to have_received(:increment).with('api.uhd.conditions.error')
    end
  end
end
