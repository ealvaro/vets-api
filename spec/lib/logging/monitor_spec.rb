...
RSpec.describe Logging::Monitor do
  ...
  let(:allowlist) { %w[user_account_uuid document_id error errors tags form_id claim_id confirmation_number fake_ssn] }
  let(:safe_keys) { %w[fake_ssn] }
  let(:monitor) { described_class.new(service, allowlist:, safe_keys:) }
