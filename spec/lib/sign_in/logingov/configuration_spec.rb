...
describe SignIn::Logingov::Configuration do
  describe '#redirect_uri' do
    ...
    context 'when review_instance_slug is not present' do
      it 'returns the logingov redirect_uri from settings' do
        expect(subject).to eq(IdentitySettings.logingov.redirect_uri)
      end
    end
  end
end
