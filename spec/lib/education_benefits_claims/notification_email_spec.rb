...
RSpec.describe EducationBenefitsClaims::NotificationEmail do
  let(:saved_claim) { create(:va10278) }
  let(:vanotify) { double(send_email: true) }

  describe '#deliver' do
    it 'successfully sends an email' do
      api_key = Settings.vanotify.services['22_10278'].api_key
      ...
      expect(vanotify).to receive(:send_email).with(
        {
          ...
          template_id: Settings.vanotify.services['22_10278'].email.error.template_id,
