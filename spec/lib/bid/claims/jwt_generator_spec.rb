...
RSpec.describe BID::Claims::JwtGenerator do
  describe '#encode_jwt' do
    it 'returns a token with required fields' do
      encoded_jwt = BID::Claims::JwtGenerator.encode_jwt
      decoded_jwt = JWT.decode(encoded_jwt, Settings.bid.claims.jwt_secret, true, {
                                 typ: 'JWT',
                                 alg: 'HS256'
                               }).first
