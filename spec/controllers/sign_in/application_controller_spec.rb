  ...
  describe '#authentication' do
    ...
    shared_context 'mpi profile validation' do
      ...
      context 'and the MPI profile has a deceased date' do
        ...
        let(:generic_error_json) { { 'errors' => 'Unauthorized' } }

        it 'renders a generic error body that does not reveal the death flag' do
          expect(JSON.parse(subject.body)).to eq(generic_error_json)
          expect(subject.body).not_to include(mpi_locked_reason)
        end
      ...
      context 'and the MPI profile has an id theft flag' do
        ...
        it 'renders a generic error body that does not reveal the theft flag' do
          expect(JSON.parse(subject.body)).to eq(generic_error_json)
          expect(subject.body).not_to include(mpi_locked_reason)
        end
