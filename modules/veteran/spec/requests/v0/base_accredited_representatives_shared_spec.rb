...
RSpec.shared_examples 'base_accredited_representatives_controller_shared_examples' do |path, type|
  context 'when find a rep is disabled' do
    before do
      allow(Flipper).to receive(:enabled?).with(:find_a_representative_enable_api).and_return(false)
    end
  ...
  context 'when find a rep is enabled' do
    before do
      allow(Flipper).to receive(:enabled?).with(:find_a_representative_enable_api).and_return(true)
    end
