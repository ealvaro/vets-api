...
module AskVAApi
  module Subtopics
    RSpec.describe Retriever do
      let(:parsed_data) do
        { Topics: [{ Id: 1, Name: 'Subtopic A', ParentId: 'topic-1', RankOrder: 0 },
                   { Id: 2, Name: 'Subtopic B', ParentId: 'topic-1', RankOrder: 0 },
                   { Id: 3, Name: 'Other Subtopic', ParentId: 'topic-2', RankOrder: 0 }] }
      end
      ...
      describe '#call' do
        ...
        context 'when using mock data' do
          ...
          it 'returns items with topic_type SubTopic' do
            expect(response.map(&:topic_type).uniq).to eq(['SubTopic'])
          end
        ...
        context 'when not using mock data' do
          ...
          context 'when sorting with RankOrder values present' do
            let(:parent_id) { 'topic-1' }
            let(:parsed_data) do
              { Topics: [{ Id: 1, Name: 'Zebra', ParentId: 'topic-1', RankOrder: 1 },
                         { Id: 2, Name: 'Apple', ParentId: 'topic-1', RankOrder: 2 },
                         { Id: 3, Name: 'Other Subtopic', ParentId: 'topic-2', RankOrder: 1 }] }
            end
          ...
          context 'when any RankOrder value is nil' do
            let(:parent_id) { 'topic-1' }
            let(:parsed_data) do
              { Topics: [{ Id: 1, Name: 'Banana', ParentId: 'topic-1', RankOrder: 2 },
                         { Id: 2, Name: 'Apple', ParentId: 'topic-1', RankOrder: nil },
                         { Id: 3, Name: 'Cherry', ParentId: 'topic-1', RankOrder: 1 }] }
            end
