...
describe RepresentationManagement::AccreditedEntities::RepresentativeOrganizationSerializer, type: :serializer do
  ...
  let(:wrapped_organization) do
    RepresentationManagement::OrganizationWithAcceptanceMode.new(organization, acceptance_mode:)
  end
  let(:data) { subject.serializable_hash.with_indifferent_access['data'] }
  let(:attributes) { data['attributes'] }

  it 'includes id' do
    expect(data['id']).to eq('XYZ')
  end
  ...
  it 'does not include acceptance_mode' do
    expect(attributes).not_to have_key('acceptance_mode')
  end

  describe 'can_accept_digital_poa_requests gating' do
    context 'when the org allows digital requests and the rep acceptance_mode is self_only' do
      let(:acceptance_mode) { 'self_only' }
    ...
    context 'when the org allows digital requests and the rep acceptance_mode is any_request' do
    ...
    context 'when the rep acceptance_mode is no_acceptance' do
      let(:acceptance_mode) { 'no_acceptance' }

      it 'is false' do
        expect(attributes['can_accept_digital_poa_requests']).to be false
      end
    end

    context 'when the org does not allow digital requests' do
      let(:acceptance_mode) { 'any_request' }

      before { organization.update!(can_accept_digital_poa_requests: false) }
