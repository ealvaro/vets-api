...
describe RepresentationManagement::AccreditedEntities::OrganizationSerializer, type: :serializer do
  ...
  let(:wrapped_organization) do
    RepresentationManagement::AccreditedOrganizationWithAcceptanceCheck.new(organization, any_request_poas:)
  end
  ...
  context 'when no reps accept any request' do
    let(:any_request_poas) { Set.new }

    it 'reports reps_can_accept_any_request as false' do
      expect(attributes['reps_can_accept_any_request']).to be false
    end
  end
end
