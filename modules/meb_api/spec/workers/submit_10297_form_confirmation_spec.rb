...
describe MebApi::V0::Submit10297FormConfirmation, type: :worker do
  ...
  context 'when claim status is ELIGIBLE' do
    before do
      template_double = double('template_id', form10297_approved_confirmation_email: 'approved_template')
  ...
  context 'when claim status is DENIED' do
    before do
      template_double = double('template_id', form10297_denied_confirmation_email: 'denied_template')
      allow(Settings.vanotify.services.va_gov).to receive(:template_id).and_return(template_double)
    end
  ...
  context 'when claim status is something else' do
    before do
      template_double = double('template_id', form10297_under_review_confirmation_email: 'under_review_template')
      allow(Settings.vanotify.services.va_gov).to receive(:template_id).and_return(template_double)
    end
