...
describe MebApi::V0::Submit225490FormConfirmation, type: :worker do
  ...
  context 'when claim status is ELIGIBLE' do
    before do
      template_double = double('template_id', form225490_approved_confirmation_email: 'approved_template')
  ...
  context 'when claim status is INPROGRESS' do
    before do
      template_double = double('template_id', form225490_offramp_confirmation_email: 'offramp_template')
      allow(Settings.vanotify.services.va_gov).to receive(:template_id).and_return(template_double)
    end

    it 'uses the offramp template id' do
      described_class.new.perform('INPROGRESS', email, first_name, user_icn)
  ...
  context 'when claim status is ERROR' do
    before do
      template_double = double('template_id', form225490_offramp_confirmation_email: 'offramp_template')
      allow(Settings.vanotify.services.va_gov).to receive(:template_id).and_return(template_double)
    end
    ...
    it 'uses the offramp template id' do
      described_class.new.perform('ERROR', email, first_name, user_icn)
  ...
  context 'when a raised error occurs during VANotify::EmailJob' do
    ...
    it 'logs the error and re-raises for Sidekiq retry' do
      ...
      expect { described_class.new.perform('ELIGIBLE', email, first_name, user_icn) }.to raise_error(VANotify::Error)
    end
  end
end
