  ...
  module PowerOfAttorneyRequestService
    class CreateRequest
          ...
          promises << Concurrent::Promise.execute do
            Datadog::Tracing.continue_trace!(trace_digest) do
              ...
              if res
                ...
                phone_data['phoneNumber'] = phone_data['phoneNumber']&.gsub(/\D/, '')
                phone_data['areaCode'] = phone_data['areaCode']&.gsub(/\D/, '') if phone_data.key?('areaCode')
      ...
      def parse_phone_data(phone_data, location_type)
        ...
        "#{phone_data[:areaCode]}#{phone_data[:phoneNumber]}".gsub(/\D/, '')
      end
    end
  end
end
