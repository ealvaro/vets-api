...
RSpec.describe ClaimsApi::BaseReloader do
  let(:reloader) { ClaimsApi::VSOReloader.new }
  let(:user_type_attorney) { ClaimsApi::VSOReloader::USER_TYPE_ATTORNEY }
  let(:user_type_claim)    { ClaimsApi::VSOReloader::USER_TYPE_CLAIM_AGENT }
  let(:user_type_vso)      { ClaimsApi::VSOReloader::USER_TYPE_VSO }
  ...
  describe '#fetch_data' do
    ...
    before do
      allow(Faraday).to receive(:new).with(url: ClaimsApi::BaseReloader::BASE_URL).and_return(conn)
    end
