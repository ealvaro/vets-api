...
class FakeTargetVeteranController < ApplicationController
...
describe FakeTargetVeteranController do
  let(:controller) { FakeTargetVeteranController.new }
  ...
  describe '#user_represents_veteran?' do
    ...
    context 'when first_name is nil' do
      before { allow(controller.current_user).to receive(:first_name).and_return(nil) }
    ...
    context 'when last_name is nil' do
      before { allow(controller.current_user).to receive(:last_name).and_return(nil) }
    ...
    context 'when no representatives are found' do
      before do
    ...
    context 'when multiple representatives are found' do
      let(:rep2) { double('Representative', poa_codes: ['C3D']) }

      before do
        allow(Veteran::Service::Representative).to receive(:all_for_user).and_return([rep, rep2])
      end
    ...
    context 'when the veteran has no POA code' do
      let(:power_of_attorney) { double('PowerOfAttorney', code: nil) }
    ...
    context "when the rep's POA codes do not include the veteran's POA code" do
      let(:veteran_poa_code) { 'Z9Z' }
