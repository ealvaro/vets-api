...
RSpec.describe 'ClaimsApi::V2::PowerOfAttorney::2122a', type: :request do
  ...
  describe 'PowerOfAttorney' do
    ...
    before do
      ...
      create(rep_factory, representative_id: '999999999999', poa_codes: [organization_poa_code])
    ...
    describe 'appoint_individual' do
      ...
      shared_examples 'appoint_individual flag-sensitive behavior' do
            ...
            context 'when valid' do
              ...
              context 'when Veteran has multiple BIRLS IDs' do
                it_behaves_like 'returns 422 for veteran with duplicate BIRLS IDs' do
                  let(:submit_path) { appoint_individual_path }
              ...
              shared_context 'claimant data setup' do
                before do
                  allow_any_instance_of(
                    claimant_web_service
                  ).to receive(:find_poa_by_participant_id).and_return(bgs_poa)
                  allow_any_instance_of(org_web_service)
                    .to receive(:find_poa_history_by_ptcpnt_id).and_return({ person_poa_history: nil })
                end
              end
      ...
      context 'when the claims accreditation tables flag is disabled' do
        include_examples 'appoint_individual flag-sensitive behavior'
      end
    ...
    describe 'validate 2122a' do
      ...
      shared_examples 'validate 2122a flag-sensitive behavior' do
        context 'CCG (Client Credentials Grant) flow' do
          context 'when provided' do
            context 'when valid' do
                ...
                context 'when the Veteran ICN is found in MPI' do
                  ...
                  context 'when the request data passes schema validation' do
                    context 'when no representatives have the provided POA code' do
                      ...
                      it 'returns a meaningful 404' do
                        mock_ccg(%w[claim.write claim.read]) do |auth_header|
                          detail = 'Could not find an Accredited Representative with ' \
                                   'registration number: 999999999999 ' \
                                   'and poa code: aaa'
      ...
      context 'when the claims accreditation tables flag is disabled' do
        include_examples 'validate 2122a flag-sensitive behavior'
      end
