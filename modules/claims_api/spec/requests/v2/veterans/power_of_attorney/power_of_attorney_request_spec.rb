...
RSpec.describe 'ClaimsApi::V2::PowerOfAttorney::PowerOfAttorneyRequest', type: :request do
  ...
  before do
    create(rep_factory, :vso, representative_id: '999999999999', poa_codes: ['067'])
    create(org_factory, poa: '067', name: 'DISABLED AMERICAN VETERANS')
    allow(Flipper).to receive(:enabled?).with(:lighthouse_claims_api_poa_dependent_claimants).and_return(false)
    allow(Flipper).to receive(:enabled?).with(:claims_api_use_claims_accreditation_tables).and_return(false)
  end

  shared_examples 'CCG flow flag-sensitive behavior' do
          ...
          context 'when the Veteran ICN is found in MPI' do
            ...
            context 'when the claimant request data does not pass schema validation' do
              ...
              it 'returns a meaningful 422' do
                VCR.use_cassette('claims_api/mpi/find_candidate/valid_icn_full') do
                  mock_ccg(scopes) do |auth_header|
                    detail = "If claimant is present 'address' must be filled in with " \
                             'required fields addressLine1, city, stateCode and countryCode. ' \
                             "If the countryCode is 'US' then zipCode is also required."
  ...
  context 'when the claims accreditation tables flag is disabled' do
    include_examples 'CCG flow flag-sensitive behavior'
  end
