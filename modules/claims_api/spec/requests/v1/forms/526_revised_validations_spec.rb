    ...
    context 'when disability name is blank' do
      ...
      it 'raises an InvalidFieldValue error' do
        expect { subject.validate_form_526_disability_name! }
          .to raise_error(Common::Exceptions::InvalidFieldValue) do |error|
            expect(error.errors.first.detail).to include('Disability name is required')
          end
      end
    end
            ...
            it 'raises an InvalidFieldValue error' do
              expect { subject.validate_form_526_secondary_disabilities! }
                .to raise_error(Common::Exceptions::InvalidFieldValue) do |error|
                  expect(error.errors.first[:detail])
                    .to include(
                      '"9999" is not a valid value for "disabilities.0.secondaryDisabilities.0.classificationCode"'
                    )
                end
            end
          end
            ...
            it 'raises an InvalidFieldValue error' do
              expect { subject.validate_form_526_secondary_disabilities! }
                .to raise_error(Common::Exceptions::InvalidFieldValue) do |error|
                  expect(error.errors.first[:detail])
                    .to include(
                      '"1111" is not a valid value for "disabilities.0.secondaryDisabilities.0.classificationCode"'
                    )
                end
            end
          end
      ...
      context 'when secondary disability has HEPC special issue without hepatitis name' do
        ...
        it 'raises an InvalidFieldValue error' do
          expect { subject.validate_form_526_secondary_disabilities! }
            .to raise_error(Common::Exceptions::InvalidFieldValue) do |error|
              expect(error.errors.first[:detail])
                .to include("specialIssues cannot include 'HEPC' unless the secondary disability name is 'hepatitis'")
            end
        end
      end
      ...
      context 'when secondary disability has POW special issue without confinements' do
        ...
        it 'raises an InvalidFieldValue error' do
          expect { subject.validate_form_526_secondary_disabilities! }
            .to raise_error(Common::Exceptions::InvalidFieldValue) do |error|
              expect(error.errors.first[:detail])
                .to include('serviceInformation.confinements is required if specialIssues includes POW')
            end
        end
      end
    end
    ...
    describe 'approximate begin date validation' do
      ...
      context 'when secondary disability has approximate begin date in the future' do
        ...
        it 'raises an InvalidFieldValue error' do
          expect { subject.validate_form_526_secondary_disabilities! }
            .to raise_error(Common::Exceptions::InvalidFieldValue) do |error|
              expected_date = 1.day.from_now.to_date.iso8601
              expect(error.errors.first[:detail])
                .to include(
                  "\"#{expected_date}\" is not a valid value for " \
                  '"disabilities.0.secondaryDisabilities.0.approximateBeginDate"'
                )
            end
        end
      end
