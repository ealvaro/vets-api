...
describe ClaimsApi::Representative, type: :model do
  let(:identity) { create(:user_identity) }
  let(:organization_class) { ClaimsApi::Organization }
  let(:organization_representative_class) { ClaimsApi::OrganizationRepresentative }

  describe 'table mapping' do
    it 'is backed by the claims_api_representatives table' do
      expect(described_class.table_name).to eq('claims_api_representatives')
    end
  end

  describe 'individual record' do
    it 'is valid with valid attributes' do
      expect(described_class.new(representative_id: '12345', poa_codes: ['000'])).to be_valid
    end

    it 'is not valid without a poa' do
      representative = described_class.new(representative_id: '67890', poa_codes: nil)
  ...
  describe 'finding by identity' do
    let(:representative) do
      create(:claims_api_representative,
             basic_attributes)
    end
    ...
    describe 'finding by the name' do
      it 'finds a user' do
        expect(described_class.for_user(
          first_name: identity.first_name,
          last_name: identity.last_name
    ...
    describe '#all_for_user' do
      it 'handles a nil value without throwing an exception' do
        expect(described_class.all_for_user(
                 first_name: identity.first_name,
                 last_name: nil,
                 middle_initial: 'J',
                 poa_code: '016'
  ...
  describe '.find_within_max_distance' do
    before do
      create(:claims_api_representative, representative_id: '456', long: -77.050552, lat: 38.820450,
                                         location: 'POINT(-77.050552 38.820450)') # ~6 miles from Washington, D.C.

      create(:claims_api_representative, representative_id: '789', long: -76.609383, lat: 39.299236,
                                         location: 'POINT(-76.609383 39.299236)') # ~35 miles from Washington, D.C.

      create(:claims_api_representative, representative_id: '123', long: -77.466316, lat: 38.309875,
                                         location: 'POINT(-77.466316 38.309875)') # ~47 miles from Washington, D.C.

      create(:claims_api_representative, representative_id: '246', long: -76.3483, lat: 39.5359,
  ...
  describe '#organizations' do
    ...
    context 'when there are organizations with the representative poa_codes' do
      it 'returns a list of those organizations' do
        organization1 = organization_class.create!(poa: 'ABC', name: 'Org ABC')
        organization2 = organization_class.create!(poa: '123', name: 'Org 123')

        expect(representative.organizations).to contain_exactly(organization1, organization2)
      end

      it 'returns ClaimsApi::Organization records (not Veteran::Service::Organization)' do
        organization_class.create!(poa: 'ABC', name: 'Org ABC')

        expect(representative.organizations).to all(be_a(organization_class))
      end
    end
  end
  ...
  describe 'callbacks' do
    describe '#set_full_name' do
      ...
      context 'updating an existing representative' do
        it 'sets the full_name attribute as first_name + last_name' do
          representative = create(:claims_api_representative, first_name: 'Joe', last_name: 'Smith')
  ...
  describe '#geocoding_record_id' do
    let(:representative) { create(:claims_api_representative, representative_id: 'test-rep-999') }
  ...
  describe 'associations' do
    ...
    it 'returns organizations through organization_representatives (integration)' do
      representative = described_class.create!(
      ...
      organization = organization_class.create!(
        poa: 'ABC',
        name: 'Test Org'
      )

      organization_representative_class.create!(
