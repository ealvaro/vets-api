...
RSpec.describe ClaimsApi::OrganizationRepresentative, type: :model do
  describe 'table mapping' do
    it 'is backed by the claims_api_organization_representatives table' do
      expect(described_class.table_name).to eq('claims_api_organization_representatives')
    end
  end

  describe 'associations' do
    it 'belongs to a ClaimsApi::Representative (keyed by representative_id)' do
      assoc = described_class.reflect_on_association(:representative)
      expect(assoc.macro).to eq(:belongs_to)
      expect(assoc.class_name).to eq('ClaimsApi::Representative')
      expect(assoc.foreign_key.to_s).to eq('representative_id')
      expect(assoc.options[:primary_key].to_s).to eq('representative_id')
    end

    it 'belongs to a ClaimsApi::Organization (keyed by poa)' do
      assoc = described_class.reflect_on_association(:organization)
      expect(assoc.macro).to eq(:belongs_to)
      expect(assoc.class_name).to eq('ClaimsApi::Organization')
  ...
  describe 'deactivation' do
    ...
    describe 'scopes' do
      it 'returns active and deactivated correctly' do
        ...
        rep2 = ClaimsApi::Representative.create!(
    ...
    describe '.deactivate!' do
      it 'bulk sets deactivated_at for the provided ids' do
        ...
        b_rep = ClaimsApi::Representative.create!(
          representative_id: 'REP124',
          first_name: 'Other',
          last_name: 'Rep',
          poa_codes: ['ABC'],
  ...
  describe 'integration sanity' do
    it 'connects a ClaimsApi::Representative and ClaimsApi::Organization via the join record' do
      ...
      org = ClaimsApi::Organization.create!(
        poa: 'AAA',
        name: 'AAA Org'
      )
