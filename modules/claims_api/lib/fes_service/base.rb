  ...
  module FesService
    class Base
      ...
      def log_outcome_for_claims_api(action, status, response, claim)
        sanitized = sanitize_error_detail_pii(response)
        ClaimsApi::Logger.log('fes_service', message: "FES #{action} #{status}: #{sanitized}",
      ...
      def sanitize_error_detail_pii(response)
        ...
        errors.each do |error|
          ...
          if error.key?(:source)
            ...
            error[:detail] = error[:detail].gsub(SPRING_FIELD_VALUE_PATTERN, SPRING_FIELD_VALUE_REPLACEMENT)
          end
        end
        sanitized.to_s
      end
