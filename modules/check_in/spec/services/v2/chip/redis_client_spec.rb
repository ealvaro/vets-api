...
describe V2::Chip::RedisClient do
  ...
  describe '#session_id' do
    it 'builds the session_id from the redis_session_prefix and tmp_api_id' do
      expect(redis_client.session_id).to eq('check_in_chip_v2_2dcdrrn5zc')
    end

    it 'uses a different cache key when the api_id is different' do
      allow(Settings.check_in.chip_api_v2).to receive(:tmp_api_id).and_return('different_api_id')
