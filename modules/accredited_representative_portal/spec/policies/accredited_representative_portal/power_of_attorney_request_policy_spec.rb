  ...
  RSpec.describe PowerOfAttorneyRequestPolicy, type: :policy do
    ...
    describe '#show?' do
      ...
      context 'when feature flag is enabled' do
        ...
        context 'when registration_numbers raises an error' do
          let(:power_of_attorney_holders) do
            [
              PowerOfAttorneyHolder.new(
                type: 'veteran_service_organization',
          ...
          before do
            allow(user).to receive(:registration_numbers).and_raise(StandardError, 'OGC API timeout')
          end

          it 'denies access gracefully instead of 500ing' do
            expect(policy.show?).to be false
          end
        end
      end
    ...
    describe 'Scope' do
      ...
      context 'when feature flag is enabled' do
        ...
        context 'when acceptance_mode is no_acceptance' do
          ...
          it 'still returns org requests (scope is broad, can_accept controls access)' do
            expect(resolved_scope).to contain_exactly(matching_request)
          end
        end
        ...
        context 'when registration_numbers raises an error' do
          ...
          it 'falls back to legacy scope instead of 500ing' do
            expect(resolved_scope).to contain_exactly(matching_request)
          end
        end
      end
