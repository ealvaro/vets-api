...
RSpec.describe AccreditedRepresentativePortal::DisableAccreditedOnlineSubmission2122Service do
  describe '.call' do
    ...
    context 'with comma-separated string' do
      ...
      let!(:org_yhz) do
        create(:accredited_organization, poa_code: 'YHZ', can_accept_digital_poa_requests: true, name: 'YHZ',
                                         primary_org_acceptance_mode: 'any_request',
                                         default_new_rep_acceptance_mode: 'any_request')
      end
    ...
    context 'when no organizations match' do
      let!(:org_other) do
        create(:accredited_organization, poa_code: 'OTH', can_accept_digital_poa_requests: true, name: 'OTHER')
      end
    ...
    context 'updates accreditation permissions for active accreditations only' do
      ...
      let!(:active_already_no_acceptance) do
        create(
          :accreditation,
          accredited_organization: org,
      ...
      it 'sets acceptance_mode=no_acceptance for active accreditations and returns accurate counts' do
        result = call_service
    ...
    context 'when org update count mismatches expected (fail loudly + rollback)' do
      ...
      it 'raises and rolls back so accreditation acceptance_mode is not changed' do
        ...
        allow(described_class).to receive(:disable_online_submission!)
