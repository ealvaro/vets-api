...
RSpec.describe AccreditedRepresentativePortal::SetOrgsAcceptanceModes2122Service do
  describe '.call' do
    context 'with valid inputs' do
      ...
      it 'updates org columns without changing reps' do
        ...
        expect(active_rep.reload.acceptance_mode).to eq('any_request')
      end
    end

    context 'with multiple orgs' do
      let!(:org_svs) { create(:veteran_organization, poa: 'SVS', name: 'SVS') }
      let!(:org_yhz) { create(:veteran_organization, poa: 'YHZ', name: 'YHZ') }
