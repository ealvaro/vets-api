...
RSpec.describe AccreditedRepresentativePortal::V0::PowerOfAttorneyRequestsController, type: :request do
  ...
  shared_examples 'power of attorney requests endpoints' do
    before do
      login_as(test_user)
      travel_to(time)
      ...
      allow_any_instance_of(AccreditedRepresentativePortal::PowerOfAttorneyHolderMemberships)
        ...
        .and_return([registration_number])
    ...
    let(:vso) { create_holder_organization(poa_code:, name: 'Test VSO', can_accept: true) }
    let(:other_vso) { create_holder_organization(poa_code: other_poa_code, name: 'Other VSO', can_accept: true) }
    ...
    describe 'GET /accredited_representative_portal/v0/power_of_attorney_requests' do
        ...
        context 'with individual accept permissions (scope filtering)' do
          ...
          context 'when acceptance_mode is self_only' do
            before do
              ...
              poa_request.update!(accredited_individual_registration_number: registration_number)
              same_org_request_one.update!(accredited_individual_registration_number: registration_number)
              same_org_request_two.update!(accredited_individual_registration_number: '999999')
            end

            it 'returns all org requests with can_accept reflecting permissions' do
              get('/accredited_representative_portal/v0/power_of_attorney_requests')
              ...
              can_accept_by_id = parsed_response['data'].to_h { |p| [p['id'], p['canAccept']] }
              expect(can_accept_by_id[poa_request.id]).to be true
              expect(can_accept_by_id[same_org_request_one.id]).to be true
              expect(can_accept_by_id[same_org_request_two.id]).to be false
            end
          end
      ...
      describe 'GET /accredited_representative_portal/v0/power_of_attorney_requests/:id' do
        ...
        context 'with individual accept permissions (record authorization)' do
          ...
          it 'returns 403 when acceptance_mode is no_acceptance' do
            create_holder_acceptance(registration_number:, poa_code:, acceptance_mode: 'no_acceptance')
          ...
          it 'returns 403 when acceptance_mode is self_only and request is assigned to someone else' do
            create_holder_acceptance(registration_number:, poa_code:, acceptance_mode: 'self_only')
  ...
  context 'with legacy models' do
    ...
    include_examples 'power of attorney requests endpoints'
  end
