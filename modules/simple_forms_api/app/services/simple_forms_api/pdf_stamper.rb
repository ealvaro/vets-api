  ...
  class PdfStamper
    ...
    def stamp_pdf
      ...
      all_form_stamps.each do |desired_stamp|
        Rails.logger.info("all_form_stamps PDF stamping for: #{stamped_template_path}")
    ...
    def perform_multistamp(stamp_path)
      ...
      begin
        ...
        if e.message.include?('ClassCastException')
          Rails.logger.warn(
            "SimpleFormsApi::PdfStamper: pdftk failed with known ClassCastException bug (#{e.class}), " \
