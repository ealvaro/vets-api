  ...
  describe 'GET /mobile/v1/health/immunizations' do
    context 'Lighthouse data retrieval' do
      ...
      context 'when the expected fields have data' do
        ...
        context 'for items that do have a location' do
          it 'has a relationship' do
            VCR.use_cassette('mobile/lighthouse_health/get_immunizations', match_requests_on: %i[method uri]) do
              get '/mobile/v1/health/immunizations', headers: sis_headers, params: { page: { size: 10, number: 1 } }
            end
            record = response.parsed_body['data'].find { |item| item['id'] == 'I2-LJAZCGMN3BZVQVKQCVL7KMTHJA000000' }
            expect(record['relationships']).to eq(
              {
                'location' => {
                  'data' => {
                    'id' => 'I2-YVJX3UTR33TUBWWMPIRJQP34FA000000',
                    'type' => 'location'
                  },
                  'links' => {
                    'related' => 'www.example.com/mobile/v0/health/locations/I2-YVJX3UTR33TUBWWMPIRJQP34FA000000'
                  }
                }
              }
            )
      ...
      describe 'record order' do
        it 'orders records by ascending date' do
          ...
          expect(dates).to eq(['2014-01-26T09:59:25Z', '2015-02-01T09:59:25Z', '2016-02-07T09:59:25Z',
                               '2017-02-12T09:59:25Z', '2018-02-18T09:59:25Z', '2019-02-24T09:59:25Z',
                               '2020-03-01T09:59:25Z', '2020-03-01T09:59:25Z', '2021-04-18T09:59:25Z',
                               '2021-05-09T09:59:25Z', '2022-03-13T09:59:25Z'])
        end
      end
      ...
      describe 'when multiple items have same date' do
        context 'date is available' do
          it 'sorts items in ascending date order then alphabetical order by group name' do
            # disabling test temporarily because it's failing in CI and docker for unknown reasons.
            # we were able to determine that this test runs three times. the first time, it passes.
            # the second and third times, it fails because response.parsed_body['data'] is nil.
            # other tests are only running once, and we have no idea why this test is running three times.
