...
RSpec.describe 'Mobile::V0::Claim::Document', :skip_json_api_validation, type: :request do
  ...
  # Shared examples for locked PDF tests
  shared_examples 'locked PDF handling' do |evidence_submission_count|
    ...
    it 'accepts locked PDFs with the correct password' do
      params = { file: locked_file, claimId: claim_id, trackedItemId: tracked_item_id,
                 documentType: document_type, password: 'test' }
      post '/mobile/v0/claim/600117255/documents', params:, headers: sis_headers
  ...
  # Shared examples for general document upload tests
  shared_examples 'general document upload tests' do |evidence_submission_count|
  ...
  context 'when cst_send_evidence_submission_failure_emails is disabled' do
    ...
    context 'with pdftk (hexapdf disabled)' do
      ...
      include_examples 'general document upload tests', 0

      context 'with locked PDF' do
        include_examples 'locked PDF handling', 0
      end
    end

    context 'with hexapdf enabled' do
      before do
        allow(Flipper).to receive(:enabled?)
          .with(:lighthouse_document_convert_to_unlocked_pdf_use_hexapdf)
          .and_return(true)
      ...
      context 'with locked PDF' do
        ...
        it 'uses HexaPDF for unlocking' do
          locked_file = fixture_file_upload('locked_pdf_password_is_test.pdf', 'application/pdf')
          params = { file: locked_file, claimId: claim_id, trackedItemId: tracked_item_id,
                     documentType: document_type, password: 'test' }

          expect(Common::PdfHelpers).to receive(:unlock_pdf).and_call_original
  ...
  context 'when cst_send_evidence_submission_failure_emails is enabled' do
    ...
    context 'with pdftk (hexapdf disabled)' do
      ...
      include_examples 'general document upload tests', 1

      context 'with locked PDF' do
        include_examples 'locked PDF handling', 1
      end
    end
