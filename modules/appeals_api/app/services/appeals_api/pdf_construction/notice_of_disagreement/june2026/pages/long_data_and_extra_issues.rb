...
module AppealsApi
  module PdfConstruction
    module NoticeOfDisagreement::June2026
      module Pages
        class LongDataAndExtraIssues
          ...
          def extra_issues?
            ...
            form_data.contestable_issues.take(Structure::MAX_ISSUES_ON_MAIN_FORM).any? do |issue|
              NoticeOfDisagreement::June2026::Structure.issue_text_exceeds_column_width?(issue)
            end
          end
          ...
          def extra_issues_table_data
            ...
            form_data.contestable_issues.take(Structure::MAX_ISSUES_ON_MAIN_FORM).select(&:text_exists?).each do |issue|
              ...
              next unless NoticeOfDisagreement::June2026::Structure.issue_text_exceeds_column_width?(issue)

              issue_atts = issue['attributes']

              data << [issue_atts['issue'], issue_atts['disagreementArea'], issue_atts['decisionDate']]
            end

            data += form_data.contestable_issues.drop(Structure::MAX_ISSUES_ON_MAIN_FORM).map do |issue|
              issue_atts = issue['attributes']
              [issue_atts['issue'], issue_atts['disagreementArea'], issue_atts['decisionDate']]
            end
