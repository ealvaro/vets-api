    ...
    module SupplementalClaims
      class ContestableIssuesController < AppealsBaseController
        ...
        def fetch_from_decision_review_service
          decision_review_service.get_supplemental_claim_contestable_issues(user: current_user,
                                                                            benefit_type: params[:benefit_type])
        end

        def fetch_from_appealable_issues_service
          appealable_issues_service.get_supplemental_claim_issues(user: current_user,
                                                                  benefit_type: params[:benefit_type])
        end
      end
    end
