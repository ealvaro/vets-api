...
RSpec.describe TravelPay::V0::ComplexClaimsController, type: :request do
  ...
  describe '#create' do
    context 'when travel_pay_appt_add_v4_upgrade feature flag is disabled' do
      before do
        allow(Flipper).to receive(:enabled?)
          .with(:travel_pay_appt_add_v4_upgrade, instance_of(User)).and_return(false)
      end

      context 'when travel_pay_enable_complex_claims feature flag is enabled' do
        before do
          allow(Flipper).to receive(:enabled?)
            .with(:travel_pay_enable_complex_claims, instance_of(User)).and_return(true)
        end
          ...
          context 'when claim and appointment services dont error' do
            before do
              ...
              allow(appts_service_double).to receive(:find_or_create_appointment)
                .with(hash_including('appointment_date_time' => params_v2['appointment_date_time']))
          ...
          context 'when params are missing' do
            context 'when required params are missing' do
              ...
              required_fields.each do |missing_field|
                it "returns bad request when #{missing_field} is missing" do
                  ...
                  # Remove the field for this test iteration
                  params.delete(missing_field)

                  post('/travel_pay/v0/complex_claims', params: { complex_claim: params }, as: :json)

                  expect(response).to have_http_status(:bad_request)
                  body = JSON.parse(response.body)
                  expect(body['errors'].first['detail'])
                    .to eq("Missing required params: #{missing_field}")
                end
              end
            end
            ...
            it 'returns bad request when all required params are missing' do
              ...
              body = JSON.parse(response.body)
              expect(body['errors'].first['detail'])
                .to eq('The required parameter "complex_claim", is missing')
            end
          end
      ...
      context 'when travel_pay_enable_complex_claims feature flag is disabled' do
        before do
          allow(Flipper).to receive(:enabled?)
    ...
    context 'when travel_pay_appt_add_v4_upgrade feature flag is enabled' do
      before do
        allow(Flipper).to receive(:enabled?)
          .with(:travel_pay_appt_add_v4_upgrade, instance_of(User)).and_return(true)
      end
      ...
      context 'when travel_pay_enable_complex_claims feature flag is enabled' do
        ...
        context 'VCR-backed integration tests' do
          it 'creates a complex claim and returns claimId using vcr_cassette' do
            VCR.use_cassette('travel_pay/submit/200_find_or_create_appt_v4', match_requests_on: %i[method path]) do
              VCR.use_cassette('travel_pay/submit/200_create_claim', match_requests_on: %i[method path]) do
          ...
          it 'returns a BadRequest response when v2 params are given' do
            VCR.use_cassette('travel_pay/submit/200_find_or_create_appt', match_requests_on: %i[method path]) do
              VCR.use_cassette('travel_pay/submit/200_create_claim', match_requests_on: %i[method path]) do
                post('/travel_pay/v0/complex_claims', params: params_v2, as: :json)
                expect(response).to have_http_status(:bad_request)
                expect(JSON.parse(response.body)['errors'].first['detail'])
                  .to eq('Missing required params: appointment_name, facility_name')
              end
            end
          end
        ...
        context 'stubbed controller behavior' do
          context 'when claim and appointment services dont error' do
            before do
              ...
              appts_service_double = instance_double(TravelPay::AppointmentsService)
              allow(appts_service_double).to receive(:find_or_create_appointment)
                .with(hash_including('appointment_date_time' => params_v4['appointment_date_time']))
                .and_return({ data: { 'id' => appointment_id } })
              # Stub the claims service
              claims_service_double = instance_double(TravelPay::ClaimsService)
              allow(claims_service_double).to receive(:create_new_claim)
