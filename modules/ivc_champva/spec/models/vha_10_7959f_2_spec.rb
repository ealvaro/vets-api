...
RSpec.describe IvcChampva::VHA107959f2 do
  ...
  describe '#metadata built from the JSON fixture (Pega payload)' do
    let(:fixture_data) do
      JSON.parse(
        Rails.root.join('modules', 'ivc_champva', 'spec', 'fixtures', 'form_json', 'vha_10_7959f_2.json').read
      )
    end
    let(:known_uuid) { 'test-uuid-10-7959f-2' }
    ...
    it 'transforms validated metadata into the S3/Pega PDF attachment metadata' do
      ...
      result = IvcChampva::DataTransformations.metadata_for_s3(
        validated.merge('attachment_ids' => %w[vha_10_7959f_2]), form.form_id
      )

      expect(result).to eq(validated.except('primaryContactInfo').merge('attachment_id' => 'vha_10_7959f_2'))
      ...
      it 'increments StatsD with tags and logs submission info including ial' do
        expect(StatsD).to receive(:increment).with(
          ...
          tags: ["form_uuid:#{form_instance.metadata['uuid']}", 'identity:', 'current_user_loa:3',
                 'current_user_ial:2', 'email_used:yes', 'form_version:vha_10_7959f_2']
        )
      ...
      it 'defaults loa to 0 and ial to 0' do
        expect(StatsD).to receive(:increment).with(
          ...
          tags: ["form_uuid:#{form_instance.metadata['uuid']}", 'identity:', 'current_user_loa:0',
                 'current_user_ial:0', 'email_used:yes', 'form_version:vha_10_7959f_2']
        )
        expect(Rails.logger).to receive(:info).with(
          'IVC ChampVA Forms - 10-7959F-2 Submission',
          form_uuid: form_instance.metadata['uuid'],
