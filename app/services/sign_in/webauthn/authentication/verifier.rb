    ...
    module Authentication
      class Verifier
        ...
        def perform
          ...
          webauthn_credential.update!(last_used_at: Time.zone.now)

          create_session!
        rescue => e
          Rails.logger.error('WebAuthn authentication verification failed', exception: e)
          raise
        end
        ...
        def validated_credential
          ValidatedCredential.new(user_verification:, credential_email:, client_config:, user_attributes: nil,
                                  device_sso: nil, web_sso_session_id: nil)
        end

        def client_config
          @client_config ||= SignIn::ClientConfig.find_by!(client_id: IdentitySettings.sign_in.vaweb_client_id)
        end
