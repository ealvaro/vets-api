...
class Form214192SubmissionEmailJob
  include Sidekiq::Job

  STATS_KEY = 'api.form214192.email'
  ...
  sidekiq_retries_exhausted do |msg, _exception|
    Rails.logger.error(
      'Form214192SubmissionEmailJob retries exhausted',
  ...
  def feature_enabled?
    return true if Flipper.enabled?(:form_4192_submission_email_notification)
  ...
  def load_claim(saved_claim_id)
    SavedClaim::Form214192.find(saved_claim_id)
  end
  ...
  def track_send_success(claim)
    ...
    Rails.logger.info('Form214192SubmissionEmailJob confirmation email sent', saved_claim_id: claim.id,
                                                                              claim_guid: claim.guid)
  end

  def handle_send_failure(error, saved_claim_id)
    StatsD.increment("#{STATS_KEY}.send_failure")
    Rails.logger.error('Form214192SubmissionEmailJob failed to send confirmation email',
                       error: error.class.name,
                       saved_claim_id:)
  end
  ...
  def notify_client
    ...
    if api_key.blank?
      ...
      Rails.logger.error('Form214192SubmissionEmailJob missing vanotify api_key')
      return nil
    end
  ...
  def submission_template_id
    ...
    Rails.logger.error('Form214192SubmissionEmailJob missing template_id for form_4192_submission_email')
    nil
  end
  ...
  def fetch_mpi_profile(claim)
    ...
    MPI::Service.new.find_profile_by_attributes(
      ...
      ssn: form_data.dig('veteranInformation', 'ssn')
    )
  end
  ...
  def log_mpi_lookup_error(claim, error)
    ...
    Rails.logger.error('Form214192SubmissionEmailJob MPI lookup error',
                       error: error.class.name,
                       saved_claim_id: claim.id)
  end
