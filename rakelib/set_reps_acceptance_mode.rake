...
namespace :vso do
  desc 'Set acceptance_mode for specific reps within an organization'
  #
  # Usage:
  #   bundle exec rake "vso:set_reps_acceptance_mode[083,self_only,12345,67890]"
  #
  # Args:
  ...
  #
  task :set_reps_acceptance_mode, %i[poa_code mode] => :environment do |_t, args|
    ...
    Rails.logger.tagged('rake:vso:set_reps_acceptance_mode') do
      Rails.logger.info("POA code: #{poa_code}, mode: #{mode}, rep IDs: #{rep_ids.join(', ')}")

      result = AccreditedRepresentativePortal::SetRepsAcceptanceMode2122Service.call(
        poa_code:, rep_ids:, mode:
      )

      Rails.logger.info("Set acceptance_mode=#{mode} for #{result[:reps_updated]} active rep-org relationship(s).")
    end
  end
end
