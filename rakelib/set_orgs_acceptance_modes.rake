...
namespace :vso do
  desc <<~DESC
    Set org-level acceptance mode columns WITHOUT touching existing reps.
    ...
    If you want to set all reps AND org columns at once, use configure_online_submission instead.
  DESC
  #
  # Usage:
  #   bundle exec rake "vso:set_orgs_acceptance_modes[083,self_only,no_acceptance]"
  #   bundle exec rake "vso:set_orgs_acceptance_modes[083 074 016,self_only,no_acceptance]"
  #
  ...
  #
  task :set_orgs_acceptance_modes, %i[poa_codes primary_mode default_rep_mode] => :environment do |_t, args|
    ...
    Rails.logger.tagged('rake:vso:set_orgs_acceptance_modes') do
      Rails.logger.info("POA codes: #{poa_codes}, primary_mode: #{primary_mode}, default_rep_mode: #{default_rep_mode}")
      Rails.logger.info('Setting org-level acceptance mode columns (reps will NOT be changed)...')

      result = AccreditedRepresentativePortal::SetOrgsAcceptanceModes2122Service.call(
        poa_codes:, primary_mode:, default_rep_mode:
      )

      Rails.logger.info("Updated org acceptance modes for #{result[:org_modes_updated]} org(s).")
    end
  end
end
