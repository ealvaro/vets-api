...
namespace :vso do
  desc 'Enable or change online submission of VA Form 21-22 for the given POA code(s)'
  #
  # Usage:
  #   bundle exec rake "vso:configure_online_submission[083,any_request,any_request]"
  #   bundle exec rake "vso:configure_online_submission[083,self_only,no_acceptance]"
  #   bundle exec rake "vso:configure_online_submission[083 074 016,any_request,any_request]"
  #
  ...
  #
  task :configure_online_submission, %i[poa_codes acceptance_mode default_rep_mode] => :environment do |_t, args|
    ...
    Rails.logger.tagged('rake:vso:configure_online_submission') do
      Rails.logger.info(
        "POA codes: #{poa_codes}, acceptance_mode: #{acceptance_mode}, default_rep_mode: #{default_rep_mode}"
      )
      Rails.logger.info('Configuring online submission for matching organization(s)...')

      result = AccreditedRepresentativePortal::ConfigureOnlineSubmission2122Service.call(
      ...
      Rails.logger.info(
        "Set acceptance_mode=#{acceptance_mode} for #{result[:reps_updated]} active rep-org relationship(s)."
      )
      Rails.logger.info(
